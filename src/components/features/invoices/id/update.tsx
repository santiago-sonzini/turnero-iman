...
import { createOrder, deleteOrderAction, OrderWithDetails } from "@/app/actions/orders";
...
interface OrderCreateFormProps {
  ...
  clientId?: string;
  initialInvoice?: OrderWithDetails; // ← nueva
}

export function InvoiceUpdateForm({ onSubmit, initialInvoice }: OrderCreateFormProps) {
  ...
  // useEffect que inicializa todo al montar
  useEffect(() => {
    if (!initialInvoice) return;

    // Reconstruir los OrderItems desde los products del OrderWithDetails
    const mappedItems: OrderItem[] = initialInvoice.products.map((p) => {
      const isCustom = !p.product?.id;
      return {
        id: `temp-${p.id}`,
        ...
        subtotal: p.subtotal,
        costAtPurchase: p.costAtPurchase,
        realCost: p.costAtPurchase,
        priceBusiness: p.unitPrice,
        category: p.product?.category ?? (isCustom ? "Manual" : ""),
        index: p.index,
        isCustom,
        customName: isCustom ? p.product?.name ?? "" : null,
      };
    });

    setOrderItems(mappedItems);
    setDiscount(initialInvoice.discount);
    setPaymentStatus(initialInvoice.paymentStatus);
    setFacturaId(initialInvoice.id);
    setIsCreated(true);

    // El cliente viene como objeto parcial, lo casteamos a Client
    // ya que ClientSelector solo necesita id y name para mostrarlo
    setSelectedClient(initialInvoice.client as unknown as Client);
  }, []);
  ...
  const clearOrderItems = () => {
    setOrderItems([]);
    // Limpiar = arrancar de cero: se ocultan las acciones de la factura creada.
    setIsCreated(false);
    setFacturaId(null);
  };
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-4 px-1 pb-28 sm:px-2">
      ...
      <div>
        ...
        {facturaId && (
          <p className="text-sm text-muted-foreground">#{facturaId}</p>
        )}
      </div>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
          ...
          <div className="w-full shrink-0 space-y-3 border-t pt-3 lg:w-72 lg:border-l lg:border-t-0 lg:pl-4 lg:pt-0">
            ...
            <div className="flex flex-col gap-2">
              {/* Fila 1: acción principal + imprimir (ícono, cuando ya está creada) */}
              <div className="flex gap-2">
                {!isCreated ? (
                  ...
                  >
                    {isUpdating ? <LoadingSpinner /> : <><Pen className="mr-2 h-4 w-4" />Actualizar factura</>}
                  </Button>
                )}
                {isCreated && (
                  <Button
              ...
              {/* Fila 2: ver / eliminar (íconos, cuando ya está creada) */}
              {isCreated && facturaId && (
                <div className="flex gap-2">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {orderItems.length > 0 && (
          <Button
            ...
            className="h-9 shrink-0 border-destructive text-destructive hover:bg-destructive hover:text-destructive-foreground"
            size="sm"
          >
