...
async function main() {
  const rawArg = process.argv[2] ?? process.env.SEED_PACK_ID ?? DEFAULT_PACK_ID;
  const parsed = parsePackId(rawArg);
  if (!parsed) {
    console.error(`❌ Pack inválido: "${rawArg}". Ver src/server/demo/packs/ids.ts.`);
    process.exit(1);
  }
  const pack = getPack(`${parsed.tipo}-${parsed.rubro}`);

  const perfilCount = await prisma.businessProfile.count();
  if (perfilCount > 0) {
    console.error(
      "❌ Ya hay un BusinessProfile en esta base — parece que ya está sembrada. " +
      "Si querés re-sembrar, vaciá la base primero (o corré esto contra una base nueva)."
    );
    process.exit(1);
  }
